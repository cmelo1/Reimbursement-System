...
function getUserInfo() {
	...
	xhttp.onreadystatechange = function() {
		...
		if (xhttp.readyState == 4 && xhttp.status == 200) {
			let user = JSON.parse(xhttp.responseText); // The response gets
														// turned into this.
	...
	xhttp
			.open("GET", 'http://localhost:8080/ERS_System/HTML/employee.do',
					true); // Sends the set values 'request' to this.
	xhttp.send();
}
...
function getTicketInfo() {
	...
	// this goes straight to the request helper.
	xhttp.open("GET",
			'http://localhost:8080/ERS_System/HTML/displayTickets.do', true); // Sends
																				// the
																				// set
																				// values
																				// 'request'
